...
func (t Training) Calories() float64 {
	return 0 // Заглушка, будет переопределена для каждого типа тренировки.
}
...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speedInMsec, 2) / (w.Height / CmInM)) *
		CaloriesSpeedHeightMultiplier * w.Weight) * (w.Duration.Minutes() / MinInHours))
}
...
func (s Swimming) meanSpeed() float64 {
	return float64(s.LengthPool*s.CountPool) / MInKm / (s.Duration.Minutes() / MinInHours)
}

// Calories возвращает количество калорий, потраченных при плавании.
func (s Swimming) Calories() float64 {
	return (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight *
		(s.Duration.Minutes() / MinInHours)
}

// TrainingInfo возвращает переменную структуры InfoMessage с информацией о проведенной тренировке.
